...
class StoryBrain {
// Step 7 - Uncomment the lines below to include storyData as a private property in StoryBrain.
// Hint: You might need to change something in story.dart to make this work.
  ...
  int _storyIndex = 0;
  // Create a Map to store routes. Parent key is the story number,
  // children keys are the two choices and their values the corresponding story numbers.
  Map<int, Map<int, int>> _storyRoutes = {
    0: {1: 2, 2: 1},
    1: {1: 2, 2: 3},
    2: {1: 5, 2: 4},
  };
  // Step 17 - Create a method called nextStory(), it should not have any outputs but it should have 1 input
  // called choiceNumber which will be the choice number (int) made by the user.
  void nextStory(int choiceNumber) {
    // Step 22 - In nextStory() if the storyNumber is equal to 3 or 4 or 5, that means
    // it's the end of the game and it should call a method called restart() that resets the storyNumber to 0.
    if (_storyIndex == 3 || _storyIndex == 4 || _storyIndex == 5) {
      resetStory();
    } else {
      // If an answer is chosen, get the next story index by choice number from the Routes Map
      _storyIndex = _storyRoutes[_storyIndex][choiceNumber];
      print(_storyIndex);
    }
  }
...
// Step 27 - Create a method called buttonShouldBeVisible() which checks to see if storyNumber is 0 or 1 or 2
